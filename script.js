...
var polygonApp = {
  ...
  createPolygon: function(points, color, name) {
  ...
  initialize: function() {
      var map,
          allCheckboxes = document.querySelectorAll('.polygon'),
          polygons = [];

      // Create imaginary bounding box for marker approximation inside polygon
      ...
      google.maps.Polygon.prototype.getApproximateCentroid = function() {
          ...
          if (google.maps.geometry.poly.containsLocation(centroid, this)) {
            ...
            for (var i = 1; i <= maxLoops; i++) {
              ...
              // West side
              testPoint = google.maps.geometry.spherical.computeOffset(centroid, (widthIncrease * i), 270);
              if (google.maps.geometry.poly.containsLocation(testPoint, this)) {
                break;
              }
            };
      ...
      // Create Polygons
      polygons.push(this.createPolygon([
      ...
      polygons.forEach(function(polygon) {
        ...
        // Draw Marker for each polygon
        new google.maps.Marker({
          position: polygon.getApproximateCentroid(),
          map: map
        });

        // Check which polygon to hide
        if (!document.getElementById(polygon.content).checked) {
          polygon.setMap(null);
        };
      });
